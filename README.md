...
- **15 themes, 89 languages** — incl. TypeScript + the web3 stack (Solidity, Vyper, Move, Cairo, Noir, Circom, Sway, Cadence, Leo); add your own `.tmTheme` or `.sublime-syntax`.
...
**89 languages** out of the box — including **TypeScript** (`.ts` `.tsx` `.mts`
`.cts`) and the **web3 stack**: Solidity, Vyper, Move, Cairo, Noir, Circom,
Sway, Cadence, Leo. Plus TOML, `.env`, INI and Dockerfile. `.mjs`, `.cjs` and
`.jsx` highlight as JavaScript. Add any `.sublime-syntax` grammar in
`~/.config/opencode/syntaxes/`.
